...
def index():
    ...
    return render_template('index.html', title='a', username=session['username'],  cur_user_id=session['user_id'],
                           news=news, users=usernames)

@app.route('/ban')
def ban():
    return render_template('ban.html', title='a')    
...
def reg():
    ...
    if form.validate_on_submit():
        ...
        for x in all_data:
            if username == x[1]:
                return render_template('reg.html', title='Регистрация', form=form, alert='Такой пользователь уже зареестрирован')
    ...
    return render_template('reg.html', title='Регистрация', form=form, alert='')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if check_password_hash(true_pass, password) and exists[0]:
            ...
            for x in all_data:
                if user_name == x[1]:    
                    return render_template('login.html', title='Авторизация', form=form, alert='Неправильный пароль')
            return render_template('login.html', title='Авторизация', form=form, alert='Такой пользователь не существует')
    return render_template('login.html', title='Авторизация', form=form, alert='')
...
def add_task():
    ...
    if form.validate_on_submit():
        ...
        return redirect("/index")
    print('!!!!!!!!!!!!')
    return render_template('add_news.html', title='Добавление задачи',  cur_user_id=session['user_id'],
...
def news_edit(news_id):
    ...
    if request.method == 'GET':
        news = NewsModel(news_db.get_connection()).get(news_id)
        return render_template('edit_news.html', cur_user_id=session['user_id'], title=news[1], content=news[2], date=news[3])
    elif request.method == 'POST':
        news = NewsModel(news_db.get_connection())
        news.update(request.form['title'], request.form['content'], request.form['date'], news_id)
        return redirect('/index')
